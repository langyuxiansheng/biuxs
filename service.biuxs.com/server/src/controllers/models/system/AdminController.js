...
controller.post('/addSysAdmin', async(ctx) => {
    ctx.body = await service.addSysAdmin(ctx.request.body, ctx.state.user);
});

//查询系统管理员
controller.get('/getSysAdminList', async(ctx) => {
    ctx.body = await service.getSysAdminList(ctx.request.query, ctx.state.user);
});

//删除系统管理员
controller.delete('/delSysAdminByIds', async(ctx) => {
    ctx.body = await service.delSysAdminByIds(ctx.request.body, ctx.state.user);
});
...
controller.put('/bindSysAdminRole', async(ctx) => {
    ctx.body = await service.bindSysAdminRole(ctx.request.body, ctx.state.user);
});
...
controller.put('/updateSysPassword', async(ctx) => {
    ctx.body = await service.updateSysPassword({ data: ctx.request.body }, ctx.state.user);
});
