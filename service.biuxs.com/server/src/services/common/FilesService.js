...
module.exports = class {
     ...
     * @param {*} param0
     */
    async uploadFile({ state, files, body }) {
        ...
        try {
            ...
            if (existsSync) { //确认成功之后再进行操作
                const data = await this.__filePromise(file, uploadPath, state.user, body); //调用文件上传方法
     ...
     * @param {*} param0
     */
    async uploadFiles({ state, files, body }) {
        ...
        try {
            ...
            if (existsSync) { //确认成功之后再进行操作
                ...
                const saveFiles = await Promise.all(fileList.map((file) => {
                    return this.__filePromise(file, uploadPath, state.user, body);
                }));
                //保存文件到数据库
     ...
     * @param {*} file
     */
    __filePromise (file, uploadPath, { userId, roleId, userName }, body) {
        return new Promise((resolve, reject) => { //异常上传,同步获取
            ...
            const data = {
                userId, //上传者id
                userName: roleId ? `系统管理员` : userName || body && body.userName || '用户', //上传者名称
    ...
    async deleteFiles({ state, body: { ids } }) {
        ...
        try {
            const { userId, isAdmin, roleId } = state.user;
            const queryData = {
                where: {
                    userId,
                    fileId: { [SOP.in]: ids },
            ...
            if (isAdmin && (roleId === config.superAdminRoleId)) { //超级管理员会获取所有的文件
            ...
            if (files && files.length) { //获取数据库里的文件数据
                if (isAdmin && (roleId === config.superAdminRoleId)) { //只有超级管理员才能真正的删除文件,普通用户为软删除
    ...
    async getFiles({ state, query }) {
        ...
        if (isAdmin && (roleId === config.superAdminRoleId)) {
