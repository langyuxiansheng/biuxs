            ...
            <template slot="column" slot-scope="{data}">
                ...
                <template v-else-if="data.col.key === 'status'">
                    <el-tag :type="$appFilters.formatTagType(data.row[data.col.key])">
                        {{ data.row[data.col.key] | formatTaskStatus }}
                    </el-tag>
                </template>
                <template v-else-if="data.col.key === 'type'">
                    {{ data.row[data.col.key] | formatTaskType }}
                </template>
...
<script>
import { getTaskList, delTaskByIds, runTaskById } from '@/http';
...
export default {
    ...
    methods: {
        ...
        async init () {
            try {
                const { data: { total, list } } = await this.$axios[getTaskList.method](getTaskList.url, { params: this.table.params });
        ...
        handleDel ({ taskId, ids }) {
            this.$confirm(`此操作将会删除此数据,是否继续?`, '提示', {
                ...
                const { code } = await this.$axios[delTaskByIds.method](delTaskByIds.url, {
                    ids: ids || [taskId], isDelete: true
                });
                if (code == 200) {
         ...
         */
        runTask ({ taskId }) {
            this.$confirm(`此操作将会运行此任务(耗时操作),是否继续?`, '提示', {
                cancelButtonText: '取消',
                confirmButtonText: '确定',
                type: 'warning',
                center: true,
                ...
                const { code } = await this.$axios[runTaskById.method](runTaskById.url, { taskId });
                if (code == 200) {
        ...
        handleSelectionChange(list) {
            this.deleteData.ids = list.map(item => item.taskId);
        },
